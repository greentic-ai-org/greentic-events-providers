...
mod tests {
    ...
    fn pack_builds_with_provider_extension() {
        let pack_root =
            Path::new(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/packs/events_provider_sms");
        ...
        assert_eq!(manifest.pack_id.as_str(), "greentic.events.provider.sms");
        ...
        assert_eq!(entry.provider_type, "events.sms");
    }
}
