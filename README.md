...
- `scripts/build_packs.sh` – builds validated `*.gtpack` artifacts via `packc` (use `cargo binstall packc --locked`; optionally set `PACKC_SERIES=0.4.` in CI to enforce a series).
